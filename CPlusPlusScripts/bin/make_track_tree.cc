...
int main(int argc, char *argv[]) {
  ...
  args.add("output", 1);
	...
	//open input file
	TFile* infile = TFile::Open(vars["input"].as<string>().c_str());
	if(!infile) {
		throw runtime_error("Could not open input file");
	}

	TTree* intree = (TTree*) infile->Get("btagana/ttree");
	if(!intree) {
		throw runtime_error("Could not get the input tree");
	}
	...
	for(long int entry=0; entry < intree->GetEntries(); ++entry) { //evt loop
		...
		for(int ijet=0; ijet<nJet; ijet++) { //jet loop
			...
			for(int itrack=Jet_nFirstTrack[ijet]; itrack<Jet_nLastTrack[ijet]; ++itrack) {//second track loop (and filling)
				...
				int trkidx = momentum_mapping.at(TagVar_trackMomentum[itrack]); //this could crash!
